...
## Overview
This project is a Bash-based tool that scans a target IP address or hostname for open ports and running services, then generates a formatted security report summarizing the findings. It is designed to grow from a simple report template into a fully dynamic scanning and reporting tool that identifies potential vulnerabilities and offers remediation recommendations.

## Purpose
This project was built for a bash scripting class, with a focus on network and device security, shell scripting fundamentals, and best programming practices such as modular function design, command-line argument handling, input validation, and use of external tools like `nmap`.
